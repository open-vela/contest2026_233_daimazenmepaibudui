...
static int audio_hw_set_volume(uint8_t volume);
  ...
  if (ctx->record_thread_valid)
    {
      ...
      /* 正常路径：stop 由别的任务调，join 等录音线程收尾（很快）。
       * 但如果 stop 是从录音线程自己里调的（比如数据回调里触发的状态切换），
       * 不能 join 自己 —— 线程会在回调返回后看到 record_stop 自己跳出循环，
       * TCB 留给下一次 audio_record_start()/stop() 回收。 */
...
int audio_set_volume(audio_context_t *ctx, uint8_t volume)
{
  ...
  ret = audio_hw_set_volume(volume);
  if (ret < 0)
    {
