...
（**不依赖板载 WiFi 模块** —— 板子的 `wifi_connect()` 在这块板子上只是置标志的空壳）。
PC 侧要按 8.1 → 8.2 → 8.3 三步走，**顺序别换**：先让防火墙别丢包，再选一条 NAT 路，最后按判据验。

#### 8.1 先决：别让防火墙把板子的包丢掉（**复现者最容易卡在这里**）

RNDIS 网卡刚建出来时，Windows 会把它归到「**未识别 / 公用**」配置文件，
防火墙于是**丢掉一切来自板子的包** —— 现象是镜像"连上过 0 次"、`ping 192.168.137.2` 不通，
而此时设备管理器和 `ipconfig` 看起来**全都是好的**，极易误判成板子的问题。两选一：

- **省事（专用演示机）**：把 Windows 防火墙**关掉**
  （`控制面板 → 系统和安全 → Windows Defender 防火墙 → 启用或关闭 Windows Defender 防火墙 → 全部关闭`）。
  ⚠️ **只在这台演示机上这么干**，别在自己的联网办公机上关。
- **规范（推荐）**：跑一次 `tools\pc_env\fix_net.cmd`（**右键 → 以管理员身份运行**）。它做三件事：
  1. 把「以太网 2」的网络配置文件设成 **Private**；
  2. 加两条入站放行规则 —— 一条绑网卡，一条绑 **`192.168.137.0/24` 网段**
     （网卡被重新枚举后配置文件会退回 Public，网段那条能继续生效）；
  3. **关掉该网卡的 IPv6** —— 板子是纯 IPv4，Windows 发的每个 IPv6 包都会让 RNDIS 驱动
     打一行 ERROR，以约 10~20 KB/s 把板子控制台刷满，把 `nsh` 的输出和命令回显全埋掉。

#### 8.2 再选一条 NAT 路（两条互斥，别同时开）

**路线 A（推荐）：Windows 自带的 NetNat —— 纯 IP 转发**

```bat
:: 先关掉 ICS、把 RNDIS 网卡设成静态 192.168.137.1/24、再建 ZhiAi NetNat，最后 ping 一下板子
tools\pc_env\netnat_on.cmd
```

推荐它的原因：**ICS 的 NAT + DNS 代理会反复失效** —— 换一次 Wi-Fi / 挪一次位置之后，
...
所以本固件**把 DNS 直接写死成 `223.5.5.5`**（`CONFIG_NETDB_DNSSERVER_IPv4ADDR=0xdf050505`），
PC 侧只需要纯 NAT 转发，**不再需要 DNS 代理**。

**路线 B：Internet 连接共享（ICS）—— 图形界面，很多人更习惯**

```
控制面板 → 网络和共享中心 → 更改适配器设置
  → 右键「WLAN」→ 属性 → 切到「共享」选项卡
  → 勾选「允许其他网络用户通过此计算机的 Internet 连接来连接」
  → 「家庭网络连接」下拉框里选「以太网 2」→ 确定
```

因为 DNS 已经写死在固件里，走 ICS 也不怕它那个代理失效，所以它同样可用。
若「共享」页是灰的、或重绑后不生效，用 `tools\pc_env\ics_on.cmd`（管理员）重绑 ——
它会把「WLAN」设成 public、「以太网 2」设成 private 再启用共享。
想从 B 换回 A，直接再跑一次 `netnat_on.cmd`。
...
**不想占串口**的话，PC 侧跑 `tools\pc_env\diag_query.py broker.emqx.io`，走 MQTT 拉一份
板子状态快照（用法见 `tools/README.md`）。

> 其它环境脚本（RNDIS 卡在 Code 10、USB 网卡睡死、串口打不开）见 `tools/README.md` 第三节的表。
...
| `sftool` 报 `The specified port 'COM4' does not exist. Available ports: No available ports`，但设备管理器里 CH343 明明是 `OK` | Windows 的**串口列表**这一层滞后 / `HKLM\HARDWARE\DEVICEMAP\SERIALCOMM` 的映射丢了 —— **端口本身是好的**（直接 `open("COM4")` 能成）。**等 10 秒重试一次**即可，不用拔插、不用换线 |
| 烧录中途报 `Failed to download stub: Io(...)` 或 `os error 31（连到系统上的设备没有发挥作用）` | 设备级失败 → **板子硬断电 10 秒**再来一次（**一次性重试，别写循环**）。⚠️ **绝对不要改 `-b` 波特率**：1 Mbaud 是唯一正确的值，降速反而更不稳 |
| 插上后设备管理器里只有一个 `未知 USB 设备（设备描述符请求失败，Code 43）`，`VID_1A86` / `VID_584E` 都不见 | 板子插在 **USB hub / 扩展坞**上了（识别方法：看该口的父设备上是不是还挂过键盘、鼠标）→ **换到主机直连的口**。同一个口上如果 CH343 和 SoC 的 USB 一起挂，基本就是这个原因 |
| 板子**完全不在 USB 总线上**（连 Code 43 都没有） | 没上电 / 线没通 / 线只能供电不能传数据 → 先看板子灯亮不亮；再**换一根线**、两头都插紧（这块板的 USB 座子很松，线一受力就掉） |
| 语音面板一直显示「录制中」但说话没反应 | 跑 `tools\pc_env\diag_query.py` 看 `ract`：`0` = 麦克风其实没在录。本 PR 之前这可能是"让路请求丢了、麦克风交出去再没收回"或"`sf32lb52_audio.c` 里 `wd_start` 返回值被丢弃导致的无上界等待"，两条都已修；若仍出现，把 `wait`/`rxt` 一并贴出来 |
