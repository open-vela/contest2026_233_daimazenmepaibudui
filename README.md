...
> ⚠️ **换了 Wi-Fi / 换了路由器之后，ICS 的 DNS 代理会失效，必须把 ② 重做一遍。**
>
> 现象很有迷惑性：`以太网 2` **仍然**是 `192.168.137.1`、`SharedAccess` 服务**也**显示 Running、
> 板子 `net_test` 里"网卡已配置"**也是** PASS，但**板子的 DNS 解析全部失败**
> （MQTT 连不上、MiMo 问不了、推送发不出）。原因：ICS 的 NAT / DNS 代理**还绑在旧网络上**。
>
> **一行判断**（在 PC 上跑，这是最直接的验尸步骤）：
>
> ```powershell
> nslookup api.day.app 192.168.137.1
> ```
>
> - 返回 IP → 代理活着；
> - `No response from server` → **代理已死**，去重做 ②，或用管理员跑
>   `Restart-Service SharedAccess -Force`。
>
> 注意：板子的 DNS 服务器**就写死在这个地址**（`CONFIG_NETDB_DNSSERVER_IPv4ADDR=0xc0a88901`），
> 所以这个代理一死，板子就"问谁都没人答"。
>
> 2026-09-13 实测：当天换过 Wi-Fi，之后板子 DNS 全线失败，查了很久才定位到是 PC 侧这个代理死了
> —— **不是板子的问题，也不是烧录的问题**。
